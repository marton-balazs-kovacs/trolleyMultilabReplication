...
output: 
  rmarkdown::pdf_document:
    ...
    fig_caption: yes
geometry: "left=2cm,right=2cm,top=1cm,bottom=1.8cm"
...
demo_table <-
  ...
  gt(groupname_col = "grouping",
     rowname_col = "Region") %>%
  fmt_number(columns = c(`Age Mean`, `Age SD`),
             decimals = 1) %>%
  cols_merge(columns = c(`Age Mean`, `Age SD`),
             pattern = "{1} ({2})") %>%
  fmt_number(columns = c(Collectivism),
             decimals = 3) %>%
  fmt_percent(columns = c(`Male %`, `Higher education %`),
              decimals = 1) %>%
  fmt_missing(
    c(Country, Collectivism,
  ...
  gt::tab_footnote(
    ...
    locations = cells_column_labels(columns = c(Collectivism))) %>%
  gt::tab_options(
    table.font.size = 10,
    # row_group.font.size = 7,
    # column_labels.font.size = 7,
...
##2.1 Effect of physical contact 
...
##2.2 Comparing the standard switch and standard footbridge dilemmas
...
##2.3 Analysing familiar participants
...
##2.4 Oxford utilitarianism Scale
...
```{r, fig.pos = "H" ,out.width = "100%", fig.cap = "Results on the Oxford Utilitarianism Scale by regions. The X axis shows the two subscales, while the Y axis shows the ratings. The center of the boxplots are the median, while the lower bound corresponds to the first quartiles (25th percentile), and the upper bound corresponds to the third quartile (75th percentile). The lower whisker represents the minimum value at no more than 1.5 times the inter quartile range from the lower bound, while the upper whisker respresents the maximum value at no further than 1.5 times the inter quartile range fro the upper bound.",echo = FALSE, message = FALSE, warning = FALSE, paged.print = FALSE }
...
plot_data <-
  ...
  mutate(item_no = as.integer(item_no),
         subscale = case_when(item_no == 1L ~ "Impartial\nBeneficence",
                              item_no == 2L ~ "Instrumental\nHarm",
                              item_no == 3L ~ "Impartial\nBeneficence",
                              item_no == 4L ~ "Instrumental\nHarm",
                              item_no == 5L ~ "Impartial\nBeneficence",
                              item_no == 6L ~ "Instrumental\nHarm",
                              item_no == 7L ~ "Impartial\nBeneficence",
                              item_no == 8L ~ "Instrumental\nHarm",
                              item_no == 9L ~ "Impartial\nBeneficence",
                              TRUE ~ NA_character_),
...
#3. Exploratory analysis on overall utilitarianism and collectivism
